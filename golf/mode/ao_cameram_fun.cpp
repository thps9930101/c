...
ao_cameram_fun::ao_cameram_fun():sync_frame2(30,sync_model::sync_frame_model2::rb_output)
{
...
//--------------------------------------------------------------------------------------
    ...
    cam.Cam_d->stream_rb = std::make_shared<sync_model::vstream_data_rb>(10);
...
void ao_cameram_fun::start_cam_num(uint32_t cam_id)
{
    ...
    if(cam_it->Cam_d->dc_thr == nullptr)
        cam_it->Cam_d->dc_thr = new std::thread(&ao_cameram_fun::Cam_Decoder,this,&cam_it->Cam_d->DC_state,cam_it->Cam_d,&Cam_vid);
...
//------------------------------------------------------------------------------------
//private
void ao_cameram_fun::Cam_Decoder(bool *state,std::shared_ptr<Cam_data> cam_d,Cam_Video *Cam_vid)
{
    ao::lm_video_data *temp;
    ...
    std::string out_path = to_string(cam_d->CamNO)+"_D.avi";

    int fourcc = cv::VideoWriter::fourcc('X','V','I','D');
    cv::VideoWriter writer(out_path, fourcc, 60, cv::Size(1920,1080));*/
...
/*
        ...
        {
            Cam_vid->Video_State.endFNo = 0;
            if(Cam_vid->Video_State.startFNo ==0 && temp->type == 1)
                Cam_vid->Video_State.startFNo = temp->frm_no +10;

            if(Cam_vid->Video_State.startFNo != 0 && Cam_vid->Video_State.startFNo <= temp->frm_no)
            {
              if(cam_d->Video_FirstOpen)
              {
                  if(Cam_vid->Video_State.start_ntp_timestamp == 0)
                      Cam_vid->Video_State.start_ntp_timestamp = temp->ntp_timestamp;
                  cam_d->cam_video_path = Cam_vid->Video_Path + "/" + to_string(cam_d->CamNO)+".mp4";
                  ret = cam_d->Cam_Video.create(cam_d->cam_video_path.c_str(),Cam_vid->vid_ctx->ctx);
                  if(ret<0)
                  {
                      Cam_vid->Video_State.save = false;
                      cam_d->video_statu = false;
                      cam_d->Video_FirstOpen = true;
                  }
                  cam_d->Video_FirstOpen = false;
                  printf("[%u]FNO start[%d] \n",cam_d->CamNO,temp->frm_no);
              }
              cam_d->Cam_Video.write_pkt_data(temp->data,temp->len);

            }
            cam_d->video_statu = true;
        }
        else
        {
            if(cam_d->video_statu == true)
            {
                if(Cam_vid->Video_State.endFNo == 0)
                    Cam_vid->Video_State.endFNo = temp->frm_no+10;

                if(Cam_vid->Video_State.endFNo == 0 || (Cam_vid->Video_State.endFNo) >= temp->frm_no)
                {
                    cam_d->Cam_Video.write_pkt_data(temp->data,temp->len);
                }
                else
                {
                    Cam_vid->Video_State.end_ntp_timestamp = temp->ntp_timestamp;
                    Cam_vid->Video_State.startFNo = 0;
                    cam_d->video_statu=false;
                    cam_d->Video_FirstOpen = true;
                    printf("[%u]FNO end[%d] \n",cam_d->CamNO,temp->frm_no);
                    cam_d->Cam_Video.release();
                }
            }
        }
...
*/
        ...
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(3));  // 延遲 1 ms
            ...
            if(cam_d->dc->get_decode_frame_GPU(gpu_frame)>=0)
            {
                ...
                /*gpu_out->download(out2);

                writer2.write(out2);*/

                //AVFrame *tmp = av_frame_alloc();
                //av_frame_ref(tmp, gpu_frame);
