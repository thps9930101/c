...
static inline void debug_dump_sync_data(const sync_data_t* s, const char* tag)
{
    if (!s) {
        printf("[SYNC DUMP][%s] sync_data = nullptr\n", tag ? tag : "");
        return;
    }

    const size_t n = s->sync_data.size();
    //printf("[SYNC DUMP][%s] streams=%zu\n", tag ? tag : "", n);

    uint32_t x[2];

    for (size_t i = 0; i < n; ++i) {
        const auto& d  = s->sync_data[i];
        // multi_video_stream 與 sync_data 一一對應
        // 方便定位來源，順便把 ring/pool 指標打出來
        const auto& vs = s->multi_video_stream[i];

        /*printf("  [%02zu] frm_no=%u\n",
               i,
               d.frm_no);*/
        x[i] = d.frm_no;
    }

    //printf("sync_data frm 1[%u] 2[%u] \n",x[0],x[1]);

    if(x[1]-x[0] !=0)
        printf("sync_data frm error 1[%u] 2[%u] \n",x[0],x[1]);
}
...
void sync_frame_model2::th_rb_output2()
{
    ...
    auto get_ring_data = [&]()
    {
        //printf("---------[sync] get data 1 --------- \n");
        vs_it->stream_rb->getTail(tmp_d);
        while (tmp_d == nullptr)
        {
            //printf("---------[sync] get data 2 --------- \n");
            if ((*(vs_it->rb_stop))) {
                //printf("---------[sync] get data 3 --------- \n");
                return false;
            }
            //printf("---------[sync] get data 4 --------- \n");
            std::unique_lock<std::mutex> _lock(*(vs_it->rb_lock));
            vs_it->rb_condi->wait(_lock);
            vs_it->stream_rb->getTail(tmp_d);
            //printf("---------[sync] get data 5 --------- \n");
        }

        vs_it->stream_rb->TailNext();//++
        //printf("---------[sync] get data 6 --------- \n");
        return true;
    };
    ...
    while (th_ctrl)
    {
        ...
        //debug_dump_sync_data(sync_data, "rb_output");
