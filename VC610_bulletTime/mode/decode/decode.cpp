...
static int create_my_decode_PktMode(AVCodecContext** dec_ctx, AVBufferRef** hw_ctx, Decoder_class_param* param)
{
    ...
    if (codec_str == "h264_cuvid" || codec_str == "hevc_cuvid") {
        // 不做重排序：輸出解碼順序（零延遲取向）
        av_opt_set_int(ctx->priv_data, "reorder_queue_size", 0, 0);

        // 低延遲/減緩緩衝（依需求調整）
//        av_opt_set_int(ctx->priv_data, "surfaces", 6, 0);
//        av_opt_set_int(ctx->priv_data, "extra_hw_frames", 0, 0);
    }
